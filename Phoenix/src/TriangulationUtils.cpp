...
earcutLayer TrisUtils::earcutTriangulation(earcutPolys& polys)
{
    std::chrono::steady_clock::time_point begin = std::chrono::steady_clock::now();
    earcutLayer layer;
    int vertex_sum = 0;
...
earcutLayer TrisUtils::earcutTriangulation(const Library& lib, const uint&& NB_THREADS)
{
    std::chrono::steady_clock::time_point begin = std::chrono::steady_clock::now();
...
earcutPoly TrisUtils::convertGdstkToEarcutPoly(const Polygon* gdstk_poly)
{
    earcutPoly poly = { {} };
...
std::pair<std::pair<float2*, uint3*>, uint2> TrisUtils::convertEarcutLayerToPointer(earcutLayer& triangulation)
{
    std::chrono::steady_clock::time_point begin = std::chrono::steady_clock::now();
...
void TrisUtils::ScaleTriangulation(Triangulation& triangulation, float& scale)
{
    auto start = std::chrono::steady_clock::now();
...
void TrisUtils::WriteLibraryToObj(const std::vector<Library>& layers, const char* filename)
{
    std::ofstream file(filename);
...
std::vector<earcutPolys> TrisUtils::ConvertSVGToEarcutLayers(const char* svg_filepath)
{
    auto start = std::chrono::steady_clock::now();
